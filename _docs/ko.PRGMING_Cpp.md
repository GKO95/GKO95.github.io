...
# C++: 클래스
본 문서에서 현재까지 절차적 및 함수형 프로그래밍에 대하여 소개하였다. 객체지향 프로그래밍(object-oriented programming; OOP)은 함수 대신 클래스와 객체 사용을 기반한다. 본 장은 C++ 프로그래밍 언어에서 객체지향 프로그래밍을 구현하기 위한 사용자 정의 클래스의 생성 및 사용 방법에 대하여 소개한다.
