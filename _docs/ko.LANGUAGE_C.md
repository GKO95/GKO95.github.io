...
* **`putchar()`**: 문자(character) 하나를 터미널에 출력한다.

* **`puts()`**: 일련의 문자들(일명 문자열; string)을 터미널에 출력한다.

* **`printf()`**: 일련의 문자들(일명 문자열; string)을 터미널에 출력하며 형식 지원이 된다.

* **`fprintf()`**: 출력 함수 `printf()`의 확장된 개념으로 스트림 선택이 가능하다.
...
* **`getchar()`**: 문자(character) 하나를 입력받는다.

* **`gets()`**: 일련의 문자들(일명 문자열; string)을 입력받는다.

* **`scanf()`**: 일련의 문자들(일명 문자열; string)을 지정된 형식에 맞게 입력받는다. 문자열을 제외한 입력을 받을 모든 데이터는 주소 연산자(`&`)가 필요하다.
