...
맴버가 정의된 순서는 구조체 크기에 영향을 줄 수 있다. 만일 맴버가 `char`-`int`-`short` 자료형 순서로 정의되었을 시, 아래의 데이터 정렬로부터 총 12바이트 크기를 갖는 구조체가 된다.
...
그러나 `char`-`short`-`int` 자료형 순서로 정의하면 구조체의 크기는 8바이트로 줄어든다. 비록 `short` 자료형이 2바이트 정렬인 관계로 `char` 자료형 맴버 사이에 1바이트 패딩이 메워지지만, `int` 자료형에 의한 4바이트 크기의 정렬 경계 내에 두 자료형을 모두 담아낼 수 있기 때문이다.
