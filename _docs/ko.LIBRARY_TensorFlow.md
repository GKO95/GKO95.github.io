...
## 텐서
텐서(tensor)란, 다중차원의 배열 형태 데이터를 의미하며 파이썬의 [넘파이](../ko.PRGMING_Python/#파이썬-넘파이)와 동일한 역할을 한다. 수학에서 [텐서](https://ko.wikipedia.org/wiki/텐서)는 차원형 데이터의 일반적인 형태로써, 스칼라(scalar)와 벡터(vector)는 각각 0과 1차원을 가지는 텐서를 지칭한다.
...
# **텐서플로우: 설치**
텐서플로우는 오픈소스 라이브러리로 [GitHub](https://github.com/tensorflow/tensorflow)에서 소스 코드를 확인할 수 있다. 그러나 본 GitHub Pages에서 소개한 또다른 라이브러리인 [OpenCV](../ko.LIBRARY_OPENCV/)와 다르게, 이번에는 이미 빌드가 완료된 라이브러리 파일을 다운로드 및 설치할 것이다. 소스 코드로부터 빌드하는 과정이 매우 골치아프며 그러할 필요도 없기 때문이다.

## 파이썬
파이썬에서 최신 버전의 텐서플로우를 사용하려면 [pip](../ko.PRGMING_PYTHON/#pip)에서 다음과 같이 패키지를 설치한다.
...
윈도우에서 텐서플로우 라이브러리를 사용하는 방법은 [여기](../ko.LIBRARY_OpenCV/#윈도우-os-1)에서 참고한다.
...
### 오토그래프
오토그래프(AutoGraph)는 `tf.function` API를 사용하여 그래프를 구축하므로써 즉시 실행 시 가지는 취약점을 보완하는 실행 방식이다. 오토그래프로 실행하기 전에 즉시 실행을 통해 우선 시험하는 것을 적극 권장한다. 그 다음 `@tf.function` 구문을 통해 함수로 [데코레이팅](../ko.PRGMING_Python/#-기호)하여 그래프를 구축한다.
...
`tf.summary` API로부터 생성된 데이터는 `_SummaryState` 스레드 객체 덕분에 내부적으로 공유되어 텐서보드 자료관리에 사용되기 때문에, 네 개의 별개의 API처럼 보이지만 전부 동일한 하나의 요약 데이터를 처리하고 있다.
...
`tf.Module` API는 학습모델 신경망의 기반이 되는 [슈퍼클래스](../ko.PRGMING_Python/#상속)이며, 간단히 모듈(module)이라고 부른다. 모듈 자체는 모델이 아니지만, 신경망 층과 모델을 구성하는데 반드시 필요한 존재이다.
...
신경망의 파라미터 역할을 하는 `tf.Variable` 객체와 하위모듈(submodule)을 클래스 속성에 [시퀀스](../ko.PRGMING_Python/#파이썬-이터러블) 형태로 저장하여 변화 양상을 추적한다. 여기서 하위모듈이란, 해당 모듈의 속성으로 선언된 또다른 모듈들을 하위모듈이라 부른다.
