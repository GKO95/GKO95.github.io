...
C# 프로그래밍 언어는 기존 C/C++ 프로그래밍 언어에 비해 매우 독특한 자료형 특징들을 확인할 수 있다. 그 중 대표적인 것이 `char` 자료형이 2바이트라는 점이다. 윈도우 운영체제는 UTF-16 문자 인코딩을 사용하는데, 이는 총 2바이트를 사용하기 때문에 정해진 크기이다.
...
## 배열
배열(array)은 동일한 자료형의 데이터를 일련의 순서로 담는 저장공간이다. 배열을 정의할 시, 자료형 뒤에 대괄호(`[]`)를 기입하여 배열을 위한 변수임을 명시해야 한다.
...
위의 배열 변수는 별도의 초기화가 이루어지지 않아 기본값으로 빈 데이터 `null`이 들어있다. 변수에 할당할 배열 객체 생성은 `new` 키워드와 함께 배열 자료형 및 크기를 지정하여 객체화한다.
...
배열 객체를 생성할 때, 초기화를 하지 않으면 모든 요소들은 배열 자료형의 기본값을 갖는다. 배열의 초기화는 중괄호(`{}`)를 사용하여 데이터를 순번에 맞게 배열 요소에 할당한다.
...
# C#: 예외 처리
예외(exception)는 잘못된 코딩이나 입력으로 인해 프로그램상 실행 불가능 코드 오류이다. 컴파일러에서 걸러지는 오류가 아니기에 정상적인 프로그램 빌드가 이루어질 수 있으나, 예외가 발생하면 프로그램은 즉시 중단된다. C# 프로그래밍 언어에서는 예외를 처리할 수 있는 `throw` 키워드, `try` 및 `catch` 블록이 존재한다. 예외 처리는 빌드된 프로그램이 예외로 인해 프로그램 실행이 중단되는 것을 방지하여 안정적으로 실행되는 것을 주목표로 한다.
...
### 예외 건네주기
`catch` 블록에서 감지한 예외는 또 다른 `try`/`catch` 블록 쌍으로 넘겨주어 나머지 예외 처리를 맡길 수 있다.
...
# C#: 파일 관리
C# 언어는 외부 파일을 읽음으로써 데이터를 불러오거나 작성함으로써 데이터를 저장할 수 있다. 본 장은 주로 외부 `.txt` 텍스트 파일을 접근하고 변경하는데 집중한다.
