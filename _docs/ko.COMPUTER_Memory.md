...
### Page fault
Page fault(한국어: [페이지 부재](https://ko.wikipedia.org/wiki/페이지_부재))는 프로세스가 working set가 아닌 물리 메모리를 참조하면 발생하는 예외(exception)이다. 그러나 page fault는 가상 메모리를 활용하는 운영체제에서 매우 흔히 발생하는 현상이며, 프로그램에서 활용할 수 있는 메모리 용량을 늘리는데 필요하다.

Page fault는 크게 두 분류로 나뉘어진다:
...
시스템의 page fault 처리자가 이를 해결하면 해당 페이지는 working set에 추가된다.
