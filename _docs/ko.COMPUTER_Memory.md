...
> 파일 핸들은 `CreateFile()` 함수로부터 반환되는데 파일 매핑 객체가 더이상 필요로 하지 않을 때까지 계속 열어두어야 한다.

`CreateFileMapping()` 함수는 그 외에도 파일 매핑 객체에 이름을 지을 수 있으며, 매핑될 파일 바이트 크기 및 read/write 권한을 지정할 수도 있다.

파일 매핑 객체의 매핑 정보는 backing store에 백업된다: 파일 매핑 객체로부터 연동된 페이지가 page out 하였을 시, 해당 변동사항은 backing store에 기록된다. 그리고 파일 매핑 객체에서 가리켰던 페이지가 다시 page in 되었으면 backing store로부터 복구된다.
