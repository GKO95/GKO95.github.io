...
# 메모리: 파일 매핑
[파일 매핑](https://ko.wikipedia.org/wiki/메모리_맵_파일)(file mapping)은 저장장치의 파일을 가상 주소 공간의 페이지 바이트마다 직접적으로 연동시키는 것이다. 프로세스는 연동된 페이지로부터 매핑된 파일의 데이터를 읽어오거나 작성하는 게 가능하다. 저장장치의 파일 뿐만 아니라 장치 및 운영체제의 리소스도 메모리에 매핑될 수 있다.
...
>
> * 간단한 프로그램 로컬 메모리 접근 및 변경이 파일 입출력을 위한 시스템 호출보다 월등히 빠르다.
>
> * 
