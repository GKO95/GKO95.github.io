...
### 배열 인덱싱
인덱싱(indexing)은 두 가지 목적을 가진다: (1) 배열 요소를 호출하거나 (2) 배열 일부를 추출한다.

> MATLAB에서는 배열의 시작이 1번이다: 이는 일반적인 프로그래밍에서 배열의 시작이 0번이란 점과 확연한 차이를 가진다!

```matlab
A = [1, 2, 3; 4, 5, 6; 7, 8, 9];

% 배열 A의 1번째 행과 3번째 열의 요소
A(1, 3)
