...
## 배열
배열(array)은 동일한 자료형의 데이터를 일련의 순서로 담는 저장공간이다. 여기서 자료형이란, 숫자나 텍스트와 같이 데이터가 어떠한 유형의 자료를 나타내는지 알려주는 요소이다. 배열의 요소는 대괄호(`[]`) 안에 입력하며, 요소나 행의 구분은 아래의 기호를 사용한다.
...
그 외에도 `zeros`, `ones`, `rand`와 같은 명령어로도 손쉽게 배열을 생성할 수 있다.
...
배열의 개별 요소가 아닌 일부분을 호출하려면 범위를 지정하는 콜론 연산자(`:`)를 사용한다.

| 구문              | 설명                                       |
|-----------------|------------------------------------------|
| `:`             | 차원의 전 범위를 호출                             |
| `x : y`         | 차원의 `x`번째부터 `y`번째까지 범위를 호출               |
| `x : intvl : y` | 차원의 `x`번째부터 `y`번째까지 범위에서 `intvl` 간격만을 호출 |
| `[x, y, z]`     | 차원의 `x`, `y`, 그리고 `z` 번째만을 호출            |

여기에 편리성을 더하여 MATLAB은 배열 차원의 마지막 번째를 가리키는 `end` 키워드룰 제공한다.
...
> MATLAB에서는 배열의 1차원은 행(row), 2차원은 열(column), 그리고 3차원은 페이지(page) 번째를 의미한다.
