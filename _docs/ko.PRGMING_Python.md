...
`SUPERCLASS`의 `__init __()`메소드가 `SUBCLASS`로 상속되었으나 `SUBCLASS`는 이미 `__init __()` 메소드를 가지고 있어 오버로딩에 의해 상속이 되지 않은 것처럼 보인다. 결국 `print(Hello World")`가 나타나지 않고 `self.attribute`이 상속에도 불구하고 오류를 일으키는 이유이다.
...
# 넘파이: 기초
[넘파이](https://numpy.org/)(NumPy)는 다차원 배열을 지원하는 매우 강력하고 유용한 파이썬 전용 라이브러리이다. 특히 메모리 효율성이 일반 파이썬 리스트 객체보다 우월하여 규모가 큰 배열을 다룰 때에는 넘파이 라이브러리는 필수항목이 된다. 이러한 특징으로 인해 넘파이는 다른 라이브러리에 있어서도 독보적인 입지를 다지는데, Matplotlib 혹은 TensorFlow와 같은 라이브러리에서도 흔히 사용된다.
...
## 넘파이 배열
넘파이 배열(array)은 파이썬의 [리스트](#리스트-객체)처럼 유사하게 동일한 자료형의 데이터를 일련의 순서대로 담는 저장공간이다. 단, 넘파이는 배열 데이터들을 메모리의 한 곳에 밀집시켜 저장하기 때문에 유연성은 떨어지나 처리속도가 월등히 빠르다.

넘파이 배열은 N-차원 배열(**N**-**D**imensional **ARRAY**)을 의미하는 `ndarray` 클래스를 핵심 데이터 구조로 갖는다. 그러나 저급 `ndarray` 생성자로는 일부 한계로 인해 넘파이 배열 객체화에 적합하지가 않다. 그 중 하나가 바로 배열 요소를 지정할 수 없다는 것이다.
...
## 넘파이 인덱싱
인덱싱(indexing)은 배열의 일부를 호출하는 과정으로, 대괄호(`[]`)를 사용한다. 파이썬의 리스트와 차이점이 있다면 각 차원에 대한 위치를 쉼표(`,`)로 구분하여 하나의 대괄호에 전부 나타낸다. 인덱싱 자체는 배열에 어떠한 영향을 주지 않으며, 단순히 해당 위치의 데이터만 보여준다.
...
### 넘파이 크기
넘파이 배열은 리스트 객체가 아니므로 `len()`를 사용할 수 없으나, `shape` 속성으로 각 차원이 갖는 크기를 알 수 있다. 
...
넘파이 배열은 `ndarray.reshape()` 메소드를 통해 원하는 크기와 차원을 갖는 넘파이 배열을 반환할 수 있다. 단, 이는 해당 배열 자체의 크기와 차원을 변경하는 게 아니라는 점을 주의해야 한다.
...
```python
import numpy as np
A = np.array([[1, 2], [3, 4]])    # 2x2 넘파이 배열
B = np.array([[5, 6]])            # 1x2 넘파이 배열
C = np.array([[7], [8]])          # 2x1 넘파이 배열
...
## 자료형
넘파이는 배열이 저장할 데이터의 자료형을 `numpy.dtype` 클래스로부터 지정할 수 있다. 그 중에서 일부 파이썬 내장 자료형은 넘파이 자료형으로 호환되어 사용할 수 있다.

| 파이썬 자료형   | 넘파이 자료형         | 설명             |
|:---------:|:----------------|----------------|
| `bool`    | `numpy.bool_`   | 논리형            |
| `int`     | `numpy.int_`    | 64비트 정수        |
| `float`   | `numpy.double`  | 배정밀도 부동소수점 실수  |
| `complex` | `numpy.cdouble` | 배정밀도 부동소수점 복소수 |
| `str`     | `numpy.str_`    | 유니코드 문자열       |
| `bytes`   | `numpy.bytes_`  | 바이트 문자열        |
