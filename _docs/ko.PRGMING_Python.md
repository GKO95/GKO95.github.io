...
대표적인 논리 조건으로써 두 데이터를 대조하는 비교 연산자(relational operator)가 있다: 데이터 간 초과 `>`와 미만 `<`, 이상 `>=`과 이하 `<=`, 일치 `==`와 다름 `!=` 관계 부합 여부에 따라 논리적 참 혹은 거짓이 반환된다.

논리 자료형의 조합에 따른 논리 연산자(logical operator)도 존재한다.
...
| `not`  | 보수     | 참을 거짓으로, 또는 거짓을 참으로 변경한다.     |
