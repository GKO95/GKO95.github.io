...
# MCU: 인터럽트
[인터럽트](https://ko.wikipedia.org/wiki/인터럽트)(interrupt)는 최상위 작업순위를 가지는 이벤트이다. 가장 우선적으로 처리해야 하기 때문에 현재 진행 중인 일반 프로그램 실행을 잠시 중단시키고 인터럽트를 우선적으로 처리한다. 임베디드 시스템에서 인터럽트의 존재는 매우 중요한 이유는 바로 멀티태스킹에 필요한 [쓰레드](https://ko.wikipedia.org/wiki/스레드_(컴퓨팅))(thread) 작업이 불가능하기 때문이다.
