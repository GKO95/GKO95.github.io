...
어플리케이션은 [`static void Main()`](#진입점)에서부터 시작한다:
...
본 문서의 대부분 코드 예시에는 `static void Main()` 함수가 직접 언급되지 않았으나, 코드들은 `main()` 함수 내에서 작성되어야만 실행된다.

### 객체 및 클래스
[객체](#c-클래스)(object 혹은 instance)는 필드(field; 맴버 변수)와 메소드(method; 맴버 함수)로 구성된 독립적인 데이터 단위이다. 그리고 해당 데이터는 맴버가 무슨 값을 갖는지 그리고 어떤 동작을 수행하는지가 정의 [클래스](#c-클래스)(class) 객체화로 생성된다. 일반적으로 맴버는 클래스에서 곧바로 사용할 수 없으며 객체화를 통해서만 접근이 가능하다.

| 맴버   | 구문              | 설명                                                                                 |
|:----------:|---------------------|---------------------------------------------------------------------------------------------|
| 필드    | `object.field`      | 데이터 값을 저장하는 맴버이다.                                                          |
| 메소드   | `object.method()`   | 데이터를 처리하거나 반환하는 맴버이다.                                           |
| 프로퍼티 | `object.property` | 필드 값을 간접적으로 반환하는 메소드를 지칭한다; 필드를 직접 접근하지 않으므로, 필드의 값이 의도치 않게 변경되는 것을 방지한다. |

### `static` 접근 한정자
`static` [접근 한정자](#접근-한정자)(access modifier) 또는 접근 지정자(access specifier) 키워드를 가지는 맴버는 객체화가 필요없이 클래스에서 바로 접근하여 사용할 수 있다. 객체를 생성하지 않아도 메소드를 사용할 수 있는 점에서 매우 편리하지만, `static` 접근 한정자가 가지는 특성으로 인해 빈번한 사용은 결과적으로 복잡한 코드를 유발한다.
