...
## 메모리 함수
C 프로그래밍 언어 중에서 문자열 관련 `string.h` 헤더 파일에는 메모리를 전문적으로 처리하는 함수들이 존재한다. 이들은 C 프로그래밍 언어에서 매우 흔히 사용되므로 개발자로써는 반드시 알아야 한다.
...
# C: 파일 관리
C 프로그래밍 언어는 외부 파일을 읽음으로써 데이터를 불러오거나 작성함으로써 데이터를 저장할 수 있다. 본 장은 주로 외부 `.txt` 텍스트 파일을 접근하고 변경하는데 집중한다.
...
## 파일 읽기
C 프로그래밍 언어는 세 종류의 파일 읽기 함수를 가졌으며, 입력 함수와 매우 유사하다.
...
## 파일 쓰기
C 프로그래밍 언어는 세 종류의 파일 쓰기 함수를 가졌으며, 출력 함수와 매우 유사하다.
