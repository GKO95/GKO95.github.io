...
2. **가상 메모리**: 덤프를 생성하는데 사용되는 물리 메모리의 복사본인 페이징 파일 크기를 선정한다. 시스템은 기본적으로 OS 드라이브에만 페이징 파일이 "시스템이 관리하는 크기"로 설정되어 있으나, 만일 각 드라이브마다 페이징 파일 여부 및 크기를 직접 지정하려면 "모든 드라이브에 대한 페이징 파일 크기 자동 관리" 체크박스를 해제한다.

    ![가상 메모리 다이얼로그 창](/images/docs/windows/bsod_advanced_memory.png)

    체크박스를 해제한 다음에는 아래의 라디오 버튼 중 하나를 선택한다.
