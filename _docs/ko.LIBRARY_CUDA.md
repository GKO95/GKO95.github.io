...
* *32비트 [레지스터](../ko.EMBEDDED_MCU/#레지스터) (32-bit register)*
    : 쓰레드를 처리하는 스트리밍 프로세서의 ALU 및 FPU에 입력되는 피연산자 데이터와 출력되는 결과 데이터를 저장하는 장치이다. 32비트 크기이면 정수형 혹은 단정밀도 부동소수점 한 개를 저장할 수 있다. 각 쓰레드마다 할당된 레지스터는 다른 쓰레드에서 접근 불가하다.

    > CUDA 성능 6.1의 *NVIDIA GeForce GTX 1070*에는 각 SM마다 64K개의 32비트 레지스터가 들어있다.
...
스트리밍 멀티프로세서는 하나 이상의 블록을 동시에 처리할 수 있다.
