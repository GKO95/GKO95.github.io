...
아래는 CUDA의 병렬 컴퓨팅에 이해를 돕기 위한 NVIDIA의 한 블로그 [게시물](https://developer.nvidia.com/blog/cuda-refresher-cuda-programming-model/)에 올라온 커널 및 스레드 계층 이미지이다.

![CUDA의 스레드, 블록, 그리고 그리드](/images/docs/cuda/cuda_kernel_execution.png)

## 스레드
스레드(thread)는 하나의 작업을 처리하는데 필요한 명령어 집합체이다. 여기서 명령어(instruction)란, 컴퓨터가 가지는 가장 기초적인 동작으로 하나 혹은 두 개의 피연산자(operand) 데이터를 가지고 산술 및 비트 연산 혹은 메모리 이동 등을 처리한다. 이런 명령어들의 집합에 의해 스레드라는 실질적인 프로그램 작업이 결국 커널을 실행시킨다.

> 하드웨어에서 각 스레드는 하나의 스트리밍 프로세서, 일명 CUDA 코어에서 처리된다. *NVIDIA GeForce GTX 1070*에는 총 1920개의 스트리밍 프로세서를 갖는다.
> 
...
1. 호스트: 스레드 개수를 커널 실행 연산자(`<<<>>>`)에 명시한다.

    ```cpp
    kernel<<<1,N>>>(dptrA, dptrB, dptrC);
    ```

2. 디바이스: 디바이스 메모리를 배열처럼 간주하여 `threadIdx`로 각 스레드에 접근한다.
...
`threadIdx`가 정의되지 않는다고 오류 메시지가 나타나더라도 NVCC 컴파일러가 필요한 헤더 파일을 자동으로 추가하기 때문에 프로젝트 빌드에는 전혀 문제가 없다. 그러나 오류 메시지가 여전히 신경쓰이면 `device_launch_parameters.h` 헤더 파일을 삽입하기를 권장한다.

## 블록
블록(block)은 동일한 커널을 실행하는 스레드의 집합체로 최대 1024개까지 수용할 수 있다. 다시 말해, 하나의 블록에 들어있는 모든 스레드는 (처리할 데이터만 다를 뿐) 전부 동일한 명령어들이 동작한다. 그러므로 블록 내의 스레드는 순차적(직렬)만이 아닌 동시(병렬)에 처리될 수 있다.
...
`blockIdx`가 정의되지 않는다고 오류 메시지가 나타나더라도 NVCC 컴파일러가 필요한 헤더 파일을 자동으로 추가하기 때문에 프로젝트 빌드에는 전혀 문제가 없다. 그러나 오류 메시지가 여전히 신경쓰이면 `device_launch_parameters.h` 헤더 파일을 삽입하기를 권장한다.

### 스트리밍 멀티프로세서
스트리밍 멀티프로세서(streaming multiprocessor; SM)는 블록을 처리하는 장치로 안에는 여러 개의 스트리밍 프로세서가 들어있어 블록의 스레드를 동시에 처리한다.
...
* *공유 메모리 (shared memory)*
    : 블록 내의 스레드 사이에 공유되는 메모리로 스레드간 협업을 가능케 한다. 또한 CUDA 코어와 동일한 함께 스트리밍 멀티프로세서에 탑재되어 데이터를 매우 빠른 속도로 저장 및 호출한다.
...
## 워프
워프(warp)는 스트리밍 멀티프로세서에서 블록을 처리하기 위해 쪼갠 32개의 스레드 묶음이다. 블록의 첫 번째 워프는 항상 0번 스레드를 시작으로, 모든 워프들은 일련의 32개 스레드를 순서대로 할당받는다. 만일 마지막 워프가 32개보다 적은 스레드를 할당받으면 나머지는 명령어를 수행하지 않는 비활성 스레드(inactive thread)로 채워진다.

> GeForce 400 시리즈(Fermi 마이크로아키텍처)에 처음 소개되었으나, 현재까지도 워프의 스레드 개수는 32개로 고정이다 ([참고](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#features-and-technical-specifications__technical-specifications-per-compute-capability)).

워프가 갖는 모든 활성 스레드는 해당 워프가 수령받은 하나의 명령어를 동시에 수행하여 병렬 컴퓨팅의 위상을 보여준다 (일명 [명령어 수준 병렬성](https://en.wikipedia.org/wiki/Instruction-level_parallelism)).
...
> CUDA 성능 6.1의 *NVIDIA GeForce GTX 1070*에는 각 SM마다 4개의 워프 스케줄러를 갖으며, 워프 하나에 32개의 스레드가 있으므로 총 128개의 프로세서와 일치한다.
...
## 배열 인덱싱
CUDA 프로젝트에서 커널이 블록 혹은 스레드를 가질 때, 디바이스 메모리는 1차원 배열처럼 데이터를 호출하였다. 만일 커널이 여러 개의 블록과 스레드를 갖는다면 디바이스 메모리에는 어떠한 배열 형태로 나타날 것인지 살펴본다. 다음 그림은 네 개의 블록과 여덟 개의 스레드, 즉 `<<<4,8>>>` 커널 실행 연산자를 가진 경우의 디바이스 메모리 배열을 보여준다.

![CUDA 블록과 스레드 조합 배열<sub><i>출처: <a href="https://www.micc.unifi.it/bertini/download/parallel/2016-2017/11_gpu_cuda_2.pdf">University of Florence</a></i></sub>](/images/docs/cuda/cuda_blockidx_threadidx.png)

그러므로 디바이스 메모리는 1차원 배열의 21번째 요소는 2번 블록의 5번 스레드로부터 처리되는 데이터이다.
...
## 스레드 협업
[스트리밍 멀티프로세서](#스트리밍-멀티프로세서)는 공유 메모리를 가져 스레드 간에 데이터를 공유하므로써 협업할 수 있다고 설명하였다. 공유 메모리에 데이터를 할당하려면 디바이스 코드에서 `__shared__` 한정자와 함께 정의한다. 단, 서로 다른 블록의 스레드 간에는 하드웨어적인 이유로 공유 메모리로 협업이 불가능하다.

```cpp
__global__ void kernel() {
    // 공유 메모리에 크기 8의 정수형 배열 "arr"을 정의한다.
    __shared__ int arr[8];
}
```

### 스레드 동기화
스레드 협업에는 동기화(synchronization)가 요구되기도 한다: `__syncthreads()` 디바이스 함수로 동시간에 동일한 작업을 진행하도록 동작 순서를 제어하므로써 스레드 협업으로 발생할 수 있는 치명적인 문제나 오류를 미연에 방지할 수 있다. 대표적인 예시로 *스레드 A*가 *스레드 B*에서 공유 메모리에 데이터를 저장하기 전에 호출하는 경우가 있다.

```cpp
__global__ void kernel() {
    // 스레드 동기화 지점: 모든 스레드가 해당 지점에 도달할 때까지 그 이상을 진행하지 않는다.
    __syncthreads();
}
```

## 커널 동기화
커널은 비동기(asynchronous)적으로 실행된다: 호스트에서 커널이 호출하여 디바이스에 실행되고 있을 동안, 호스트는 기다림 없이 다음 코드로 계속 진행한다. 커널을 동기화는 `cudaDeviceSynchronize()` 호스트 함수로 이전 CUDA 커널들의 실행이 모두 종료될 때까지 호스트 코드 진행을 잠시 정지시킨다.
...
int main() {
    ...
    // 이미 실행되고 있는 kernel1 및 kernel2 커널 종료를 기다린다.
...
그 외에도 다음 함수들은 커널 동기화에 영향을 미친다.

* `cudaMemcpy()`
    : 이전에 실행된 CUDA 커널들이 모두 종료한 이후, 메모리 복사가 마무리 될 때까지 호스트 진행을 멈춘다.

* `cudaMemcpyAsync()`
    : 비동기식, 즉 호스트 진행을 멈추지 않은 상태에서 메모리를 복사한다.
