...
## 커널 동기화
커널은 비동기(asynchronous)적으로 실행된다: 호스트에서 커널이 호출하여 디바이스에 실행되고 있을 동안, 호스트는 기다림 없이 다음 코드로 계속 진행한다. 커널을 동기화는 `cudaDeviceSynchronize()` 호스트 함수로 이전 CUDA 커널들의 실행이 모두 종료될 때까지 호스트 코드 진행을 일시정지한다.
