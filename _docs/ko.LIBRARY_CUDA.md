...
## 쓰레드 협업
[스트리밍 멀티프로세서](#스트리밍-멀티프로세서)는 공유 메모리를 가져 쓰레드 간에 데이터를 공유하므로써 협업할 수 있다고 설명하였다. 공유 메모리에 데이터를 할당하려면 디바이스 코드에서 `__shared__` 한정자와 함께 정의한다. 단, 서로 다른 블록의 쓰레드 간에는 하드웨어적인 이유로 공유 메모리로 협업이 불가능하다.
...
### 쓰레드 동기화
쓰레드 협업에는 동기화(synchronization)가 요구되기도 한다: `__syncthreads()` 디바이스 함수로 동시간에 동일한 작업을 진행하도록 동작 순서를 제어하므로써 쓰레드 협업으로 발생할 수 있는 치명적인 문제나 오류를 미연에 방지할 수 있다. 대표적인 예시로 *쓰레드 A*가 *쓰레드 B*에서 공유 메모리에 데이터를 저장하기 전에 호출하는 경우가 있다.

```cpp
__global__ void kernel() {
    // 쓰레드 동기화 지점: 모든 쓰레드가 해당 지점에 도달할 때까지 그 이상을 진행하지 않는다.
    __syncthreads();
}
```
