...
## 어플리케이션
[어플리케이션은](https://ko.wikipedia.org/wiki/응용_소프트웨어)(application)은 운영체제의 [사용자 공간](/docs/ko.Linux#보호-링)에서 특정 작업을 수행하기 위해 설계된 프로그램으로 윈도우 OS에서는 흔히 `.EXE` 확장자를 갖는 실행 파일이 해당한다. [C](/docs/ko.C)/[C++](/docs/ko.Cpp)와 같은 고급 프로그래밍 언어로부터 CPU의 연산 및 동작을 직접 제어할 수 있는 [명령어](https://ko.wikipedia.org/기계어)(혹은 기계어)의 집합체로 빌드된 "실행 가능한 소프트웨어"이다.

## 프로세스
[프로세스](https://ko.wikipedia.org/wiki/프로세스)(process)는 어플리케이션을 RAM 물리 메모리로 불러와 실행하는 객체(instance)이다.

> CPU가 직접적으로 접근이 가능하며 거리가 가까운 RAM 메모리에서 처리하는 게 HDD나 SSD와 같은 저장장치에서 실행하는 것보다 매우 빠르기 때문이다.

각 프로세스마다 사용하는 물리 메모리는 [가상 주소 공간](/docs/ko.Memory#가상-주소-공간)(virtual address space)이란 고립된 주소 공간을 사용한다. 이는 한 프로세스에 문제가 발생하여도 타 프로세스에 영향을 주지 않는 안전 장치 역할을 한다. 가상 주소 공간에는 아래의 리소스들이 담겨있다:

* 어플리케이션 및 라이브러리 파일
* 스택(stack) 및 힙(heap) 영역 메모리

그 외의 프로세스 관련 정보들은 일반 프로세스가 접근할 수 없는 커널 공간으로 보호된 RAM 영역의 [프로세스 제어 블록](https://ko.wikipedia.org/wiki/프로세스_제어_블록)(process control block; PCB) 데이터 구조에 저장되어 있다.

* 프로세스에 할당된 운영체제의 리소스 서술자 (예. [파일 서술자](https://ko.wikipedia.org/wiki/파일_서술자) 혹은 [핸들](https://ko.wikipedia.org/wiki/핸들_(컴퓨팅)))
* 프로세스 보안 속성 (예. 소유자 및 권한 정보)
* [프로세서 상태](#프로세스-상태) 및 [문맥](#문맥-교환)

### 프로세스 상태
[프로세스 상태](https://en.wikipedia.org/wiki/Process_state)(process state)는 프로세스 제어 블록의 정보 중 하나로 현 프로세스가 어떤 상태에 있는지 나타낸다. 이는 운영체제가 해당 프로세스를 어떻게 처리할 것인지 결정하는 요인으로도 작용하기 때문에 프로세스를 이해하는데 도움이 된다. 아래는 프로세스가 가질 수 있는 상태에 대한 설명, 그리고 전환되는 경우를 소개한다.
...
## 문맥 교환
[문맥](https://en.wikipedia.org/wiki/Context_(computing))(context)이란, 프로세스나 스레드 등이 중단된 시점에서부터 작업을 재개할 수 있도록 필요한 최소한의 정보들이다: 스레드 스택 및 [CPU 레지스터](https://ko.wikipedia.org/wiki/프로세서_레지스터)가 이에 해당한다. 그리하여 문맥 교환(context switch)이란, 프로세스나 스레드를 나중에 재개하여 실행할 수 있도록 문맥을 저장하는 행위이다. 스케줄링과 함께 단일 코어 CPU에서 멀티태스킹을 가능하게 만드는 핵심 기능이다.
...
## 스레드 유예
스레드는 타 스레드의 실행을 유예(suspend) 및 재개(resume)시킬 수 있다; 다시 말해, 타 스레드의 상태를 [대기](#프로세스)(waiting)로 전환하거나 반대로 대기 상태를 해제할 수 있다는 의미이다. 유예된 스레드는 스케줄링에서 제외된다.
...
스레드 풀(Thread pool)은 스레드를 작업할 객체들을 미리 한군데 모아둔 것을 가리킨다. 어플리케이션에서 생성하여 사용될 전체 스레드 개수를 줄이되 스레드 작업자들을 관리하므로써 효율성을 추구하는 목적으로 사용된다. 스레드 생성 및 파괴 과정에서 발생하는 overhead도 줄일 수 있는 장점을 갖는다.
