...
파이썬 코드를 실행하기 위해서 가장 필수적인 인터프리터는 파이썬 공식 웹사이트에서 [다운로드](https://www.python.org/downloads/) 할 수 있다. 웹사이트에는 여러 버전의 파이썬이 존재하는데, 그 중에서 파이썬3는 `3.X.y` 버전 번호를 가진다. `X`는 하위버전을 의미하며, 들어가는 숫자에 따라 적용되는 문법이나 기능이 약간이라도 차이가 날 수 있다. 그리고 `y`는 소소한 버그를 수정하는 등의 개정판을 의미하므로, 숫자가 클수록 더 안정된 인터프리터라고 볼 수 있다.
