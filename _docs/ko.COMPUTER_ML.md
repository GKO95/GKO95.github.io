...
## 딥러닝
역전파 알고리즘은 기계학습에 새로운 부흥을 일으켰으나, 당시에는 시그모이드 함수(sigmoid function) 혹은 쌍곡탄젠트(hyperbolic tangent) 활성함수 위주의 신경망을 사용하였다. 결과적으로 더 많은 신경층을 쌓으면 역전파 과정에서 피드백이 사라지는 경사소실(vanishing gradient) 문제에 부딪히게 되었다. 다시 말해, 모델 학습이 이루어지지 않는 한계점에 도달하였다.

경사소실 문제는 차후 새로운 ReLU 활성함수 및 적절한 가중치 매개변수의 초기화가 소개되면서 해결되었다. 이 시점부터 더 많은 신경층을 쌓을 수 있는 "깊은" 신경망으로 모델을 학습한다 하여 *딥러닝(deep learning)*이란 용어가 탄생하였다. 결국 딥러닝은 인공신경망과 별 다를 게 없으며, 단지 신경층을 이전보다 많이 쌓아올릴 수 있을 뿐이다.
