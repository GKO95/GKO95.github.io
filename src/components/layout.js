...
const Layout = ({children}) => {
    ...
    let config = new class {
        constructor()
        {
            if (isNaN(this.GetCONFIG())) { this.SetCONFIG(0); } if (window.localStorage.length !== 1) { let config = this.GetCONFIG(); 
            if (config > 0xFF) { config &= 0xFF; } else if (config == null || isNaN(config)) { config = 0; } window.localStorage.clear(); 
