---
slug: /docs/ko.PRGMING_JavaScript
title: 프로그래밍 | 자바스크립트
...
![그림 2. 문서 객체 모델 <sub><i>출처: <a href="https://commons.wikimedia.org/wiki/File:DOM-model.svg">위키백과</a></i></sub>](../images/javascript/js_html_dom.png)
...
# **REACT: 소개**
React는 자바스크립트 라이브러리 중 하나로 사용자 인터페이스와 같은 프런트엔드를 [관심사 분리](https://ko.wikipedia.org/wiki/관심사_분리)(separation of concerns)로 개발할 수 있도록 한다. 가장 기초적인 웹사이트 구성은 디자인(HTML 및 CSS)과 상호작용(자바스크립트) 요소로 나누지만, 관심사 분리는 전혀 다른 방식으로 접근한다. 대표적인 동영상 플랫폼인 YouTube를 예로 들면 관심사는 동영상 플레이어, 연관 동영상 목록, 검색란, 댓글 등이 있다; React는 이러한 관심사들을 컴포넌트(component)라는 개별 구성요소로 다루지만, 각 컴포넌트가 가지는 디자인과 상호작용 내용은 하나로 통합되있다.

라이브러리 파일은 `production.min.js`과 `development.js`로 나뉘어지는데, 전자는 압축된 배포용이며 후자는 개발용으로 사용된다. React 라이브러리를 스크립트로 불러오는 방법은 두 가지가 있다:

* 첫 번째로는 React 라이브러리를 설치하여 사용하는 것이다. 설치하기 위해서는 [Node.js](https://nodejs.org/) 소프트웨어가 반드시 필요하다. Node.js은 자바스크립트 런타임 환경으로 웹브라우저 외에서도 자바스크립트를 사용할 수 있도록 한다. 그 중에서 npm이란 소프트웨어는 자바스크립트 모듈 패키지를 관리하는데 사용되며, 이를 통해 React 라이브러리 또한 설치할 수 있다.

  Node.js 패키지는 일반적으로 로컬에서만 적용된다. 다시 말해, 설치한 패키지는 해당 경로에서만 사용할 수 있다. 그러므로 우선 React 프로젝트를 진행하고자 하는 경로로 이동을 한 다음, 아래의 명령어로 Node.js 패키지 환경 구축한다.

  ```bash
  npm init
  ```

  `package.json` 파일이 생성되는데, 해당 JSON 파일은 현재 위치한 Node.js 환경에 설치된 자바스크립트 모듈 패키지 및 버전을 나열한다. React를 사용하기 위해 다음 두 패키지를 설치한다.

  ```bash
  npm install react react-dom --save
  ```

* 두 번째 방법으로는 온라인에서 React 라이브러리만 전달하는 CDN을 통해 불러올 수 있다. 다음과 같이 코드를 `<head>` 태그 아래에 입력하여 React 17 배포용 라이브러리를 UNPKG 서버에서 직접 불러온다.
...
## 가상 DOM
React는 가상 DOM(virtual DOM)을 사용하여 매우 빠르다는 장점을 지니고 있다. 기존 페이지의 DOM에 변화가 생기면 React에서 임시로 가상 DOM을 빠른 속도로 생성한다. 그리고 이전 DOM과 가상 DOM을 비교하여 변화가 발생한 콘텐츠만 새로 업데이트한다.

## Project Structure
`public` 폴더는 contains files related to how the application will display on the client, the most important of those being index.html, which is the HTML template of our page:

The `src` 폴더는 contains all of the JavaScript, CSS, and image files that will be compiled into a bundle file and injected into index.html


# REACT: 기초

## JSX
JSX는 자바스크립트의 확장된 구문으로 HTML 요소를 문자열이 아닌 있는 그대로 받아들일 수 있다. 또한 중괄호 `{}`를 사용하여 자바스크립트 상수 혹은 변수를 HTML 요소에 삽입도 가능하다.
...
let element = (
    <div id={variable.id}>
        <span>Hello {variable.content}</span>
    </div>
);
```

React에서는 JSX 사용을 권장하는데, 이는 JSX가 React가 추구하는 "관심사 분리"에 가장 적합하기 때문이다; 자바스크립트 하나로 각 구성요소의 UI 및 기능성을 한군데에서 관리할 수 있다. 그리고 컴파일된 JSX는 React의 각 컴포넌트를 의미하는 자바스크립트 객체가 되어 HTML DOM을 구축하는데 사용된다.
...
`props` itself is just an object that contains attribute and its value as its properties. An important thing to consider is that `props` are read-only, meaning components cannot modify their `props`.
...
First, we need to import the useState hook. `useState` returns a pair, the current state value and a function, that lets you change the state.
...
In the example above, we create a name state variable and a setName function. The square brackets syntax is called [array destructuring](#배열-구조-분해).
...
## Lifecycle
> EXCLUSIVE FOR CLASS COMPONENTS!
...
The `componentDidMount` method is called when a component is rendered on the page. (similar to class constructor)
...
```
Similarly, the `componentWillUnmount()` lifecycle method is called right before the component is removed from the DOM. (similar to class destructor/finalizer)

Another lifecycle method is `componentDidUpdate()`, which is called when a component is updated in the DOM. componentDidUpdate() is only called when the component is updated, and doesn't include when mounted or unmounted.
...
여기서 `ReactDOM.render()` 메소드는 JSX와 해당 JSX 콘텐츠 container 요소를 인자로 받는다. 여기서 `render()`를 사용하면 container 요소 내의 기존의 콘텐츠는 모두 JSX 콘텐츠로 교체된다. 그렇지만 이는 HTML 요소를 전부 교체하는게 아니라 변경 사항이 있는 콘텐츠만 새로 업데이트한다.
